...
const DBUploadFileTable = ({ table_data }) => {
  ...
  const columns = useMemo(
    () => [
      { accessor: 'comp_file_id', Header: 'ID', show: false },
      {
        accessor: 'file_title',
        Header: '파일 이름',
        Cell: ({ value }) => {
      ...
      },
      {
        ...
        Cell: ({ value, row }) => {
          return (
            <div>
              {row.original.depth1} {row.original.depth2} {row.original.depth3}{' '}
              {row.original.depth4}
            </div>
          );
        },
...
export default DBUploadFileTable;
