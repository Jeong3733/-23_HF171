...
public class EvaluationService {
    ...
    public void addEvaluation(ReqAddEvaluation reqAddEvaluation) {
        PostInfo postInfo = postInfoRepository.findById(reqAddEvaluation.getPost_id())
                .orElseThrow(() -> {
                    log.error("PostId : {}의 파일이 존재하지 않습니다.", reqAddEvaluation.getPost_id());
    ...
    public void saveAllScoreList(List<EvaluationScoreDto> evaluationScoreDtoList) {
        evaluationScoreDtoList.forEach(evaluationScoreDto -> {
            ...
            EvaluationScore evaluationScore = evaluationScoreRepository.findByPostInfoIdAndEvaluationInfoIdAndJudgeInfoId(postId, evalId, judgeId)
                    .orElseThrow(() -> {
                        log.error("PostId : {}, EvalId : {}, JudgeId : {}의 평가점수가 존재하지 않습니다.", postId, evalId, judgeId);
                        throw new EntityNotFoundException("해당 PostId, EvalId, JudgeId의 평가점수가 존재하지 않습니다.");
                    });

            EvaluationScore newEvaluationScore = EvaluationScore.builder()
                    .postInfo(postInfo)
            ...
            evaluationScoreRepository.save(newEvaluationScore);
        });
    }

    @Transactional
    public void updateAllScoreList(List<EvaluationScoreDto> evaluationScoreDtoList) {
        evaluationScoreDtoList.forEach(evaluationScoreDto -> {
            int postId = evaluationScoreDto.getPost_id();
            PostInfo postInfo = postInfoRepository.findById(postId)
                    .orElseThrow(() -> {
